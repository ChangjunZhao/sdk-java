...
import com.huawei.openstack4j.openstack.vpc.domain.Subnet;
import com.huawei.openstack4j.openstack.vpc.domain.Subnet.Subnets;
import com.huawei.openstack4j.openstack.vpc.domain.SubnetCreate;
...
public class SubnetService extends BaseVpcServices{
	 ...
	 * @return
	 */
	public Subnet create(SubnetCreate creation){
		...
		Preconditions.checkNotNull(creation.getGatewayIp(),"parameter `creation.gatewayIp` should not be empty");
		Preconditions.checkNotNull(creation.getVpcId(),"parameter `creation.vpcId` should not be empty");
		return post(Subnet.class, uri("/subnets")).entity(creation).execute();
	}
	...
	public Subnet update(Subnet subnet){
		...
		Preconditions.checkNotNull(subnet.getName(), "parameter `creation.name` should not be empty");
